...
Arrange these to be served through a server like `nginx`. The following conf file is for a `nginx` server running on localhost on port 9000. 
...
`~ ᐅ cat /usr/local/etc/nginx/nginx.conf`
...
http {
    ...
    server {
        ...
        location /eip712demo {
          alias /Users/anomit/workspace/api-usage-examples/EIP-712;
        }
    
}
```

Now the frontend can be accessed through the browser
...
Visit either `sign_flat.html` or `sign_nested.html`. [According to our `nginx` example](eip712.md#setting-up-the-frontend) they will be available at
* http://localhost:9000/eip712demo/sign_flat.html
* http://localhost:9000/eip712demo/sign_nested.html
